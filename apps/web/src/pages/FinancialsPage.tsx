  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[24px] font-bold text-slate-900 tracking-tight leading-none">{t('nav.financials')}</h1>
          <p className="text-[13px] text-slate-400 mt-1.5">{t('financials.subtitle')}</p>
        </div>
      </div>
      ...
      {can.editClubFinancials ? (
        ...
        <Card className="p-12 text-center">
          <p className="text-[15px] font-medium text-slate-900">{t('financials.restrictedTitle')}</p>
          <p className="text-[13px] text-slate-500 mt-2 max-w-md mx-auto">
            {t('financials.restrictedBody')}
          </p>
        </Card>
      )}
    </div>
