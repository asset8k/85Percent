...
export function RunProgress({ initial, targetId }: { initial: Detail; targetId: string }) {
  ...
  return <div>
    ...
    {missingItems.length > 0 && <section className="mt-7"><h2 className="mb-3 font-semibold">Removed (not found in this source)</h2><div className="space-y-3">{missingItems.map((change) => <article key={String(change.id)} className="rounded-lg border border-border bg-muted/30 p-4 text-sm"><strong>{String(change.entity_type)}</strong><p className="mt-1 text-muted-foreground">{String(change.reason ?? 'The player was not present in this source response and was removed from the roster.')}</p><details className="mt-3"><summary className="cursor-pointer text-xs font-medium text-primary">Compare source values</summary><dl className="mt-2 space-y-1 text-xs"><div><dt className="inline font-medium">Removed: </dt><dd className="inline text-muted-foreground">{formatReviewData(change.before_data)}</dd></div></dl></details></article>)}</div></section>}
